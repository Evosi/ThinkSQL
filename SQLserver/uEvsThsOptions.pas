...
type
  ...
  ['{0012F0B0-2791-4542-8F8D-5DEBAA3CC1A7}']
    ...
    procedure WriteInteger (const aKey:string; const aValue:Integer);               {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    procedure WriteDouble  (const aKey:String; const aValue:Double);                {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    procedure WriteDateTime(const aKey:String; const aValue:TDateTime);             {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    procedure Write        (const aKey:String; const aBuffer; const aSize:integer); {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};

    function ReadString   (const aKey:String):wideString;                                   {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    function ReadInteger  (const aKey:string):Integer;                                      {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    function ReadDouble   (const aKey:String):Double;                                       {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    function ReadDateTime (const aKey:String):TDateTime;                                    {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
    function Read         (const aKey:String; var aBuffer; const aBufSize:integer):Integer; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
...
procedure TEvsIniPersister.WriteInteger(const aKey :string;const aValue :Integer); {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
...
procedure TEvsIniPersister.WriteDouble(const aKey :String; const aValue :Double); {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
...
procedure TEvsIniPersister.WriteDateTime(const aKey :String;const aValue :TDateTime); {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
...
procedure TEvsIniPersister.Write(const aKey :String; const aBuffer; const aSize :integer); {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
var
...
function TEvsIniPersister.ReadString(const aKey :String) :wideString; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
    Result := FFile.ReadString(FSection,aKey,'');
  finally
    Unlock;
  end;
end;

function TEvsIniPersister.ReadInteger(const aKey :string) :Integer; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
    Result := FFile.ReadInteger(fSection,aKey,0);
  finally
    Unlock;
  end;
end;

function TEvsIniPersister.ReadDouble(const aKey :String) :Double; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
    Result := FFile.ReadFloat(FSection, aKey, 0);
  finally
    Unlock;
  end;
end;

function TEvsIniPersister.ReadDateTime(const aKey :String) :TDateTime; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
begin
  Lock;
  try
    Result := FFile.ReadDateTime(FSection,aKey,0);
  finally
    Unlock;
  end;
end;

function TEvsIniPersister.Read(const aKey :String; var aBuffer;const aBufSize :integer) :Integer; {$IFDEF WINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
...
begin
  ...
  if NewInstance<>nil then TEvsInterfacedObject(NewInstance).FRefCount:=1;
end;
...
begin
  Lock;
  try
    FDefaultDB  := aPersister.ReadString ('DatabaseName');
    DefaultExt  := aPersister.ReadString ('DefaultExtension');
    FDataFolder := aPersister.ReadString ('DataFolder');
    FServiceName:= aPersister.ReadString ('ServiceName');
    FServerName := aPersister.ReadString ('ServerName');
    FPort       := aPersister.ReadInteger('Port');
  finally
    Unlock;
  end;
end;
